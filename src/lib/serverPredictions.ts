...
export type EzbotServerClient = {
  getPredictions: (
    params: Readonly<PredictionsParams>,
...
export function buildQueryParams(params: Readonly<PredictionsParams>): string {
...
export function createEzbotClient(
  options: Readonly<EzbotServerClientOptions>
): EzbotServerClient {
  const {
  ...
  const agent = isHttpsBase
    ...
    : new HttpAgent({
        ...
        maxSockets: 50,
        maxFreeSockets: 10,
        ...(agentOptions as ConstructorParameters<typeof HttpAgent>[0]),
      });

  async function makeRequest<T = unknown>(
    url: string,
    meta?: Readonly<RequestMeta>
  ): Promise<{ status: number; statusText: string; json: () => Promise<T> }> {
  ...
  return {
    async getPredictions(
      ...
      try {
        const response = await makeRequest<PredictionsResponse>(fullUrl, meta);
